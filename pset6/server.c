...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            if(method != NULL || request_target != NULL || http != NULL || strstr(line, "\r\n") == 0 )
            {
            ...
            char* query = strchr(request_target,'q');
            if(query == NULL)
                query = "";

            // concatenate root and absolute-path
            char path_root[strlen(root) + 1];
            char path_abs[100];
            memset(path_root,'\0', sizeof(path_root));
            memset(path_abs,'\0', sizeof(path_abs));
            strcpy(path_root, root);
                         
            char path[strlen(path_root)+ strlen(path_abs)+ 1];
            memset(path, '\0', sizeof(path));
            strcpy(path, path_root);
            strcat(path, path_abs);
